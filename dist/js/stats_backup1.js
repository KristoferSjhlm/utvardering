...
(function(){
    'use strict';
        ...
        var counter = 0;
        var selectHTML = '<option value="Välj">Välj</option>';
        ...
        _.when(ds.fetch()).then(function() {
            ...
            document.getElementById('classes').innerHTML = "Klass: " + klassString;
              //ta bort härifrån
                ...
                // get the data we need filtered by Klass
                // OBS satt till bara en
                for (var i = 0; i < 1; i++) {

                    className[i] = classes.column('Klass').data[i];
               
                    // filter the data on first parameter
                    className[i] = this.where({
                    ...
                    console.log(className[i].length, className[i].toJSON());



                    // start from here. Get the url parameter function from evalutate.

                    classLessons[i] = className[i].countBy('Lektionsmal');

                    
                    for (var j = 0; j < classLessons[i].length; j++) {

                        classNameLessons[counter] = className[i].where({
                            // copy over the one column
                            columns: ['Tidstämpel', 'Klass', 'Lektionsmal', 'Maluppfyllnad'],
                            // and only where the values are > 1
                            rows: function(row) {
                                return row.Lektionsmal == classLessons[i].column('Lektionsmal').data[j]
                            }
                        });
                        ...
                        // kolla över och ta bort Klass. Ersätt med dag?
                        // adds an option-value to the select box for each Klass and Lektionsmal
                ...
                document.getElementById('select').innerHTML = selectHTML;
                 ...
                 // starting to check how to fix those arrays where not all values 1-5 is represented
                 /**
                console.log(classNameLessons[1].length);
                console.log(classNameLessons[1]);
                console.log("data " + classNameLessons[1].countBy("Maluppfyllnad").toJSON())
                
                for (var i = 0; i < classNameLessons.length; i++) {
                  if (classNameLessons[i].countBy("Maluppfyllnad").length != 5) {
                    var tempArray = classNameLessons[i].countBy("Maluppfyllnad");
                    for (var j = 0; j < 5; j++) {
                      console.log(classNameLessons[i].column('Maluppfyllnad').data[j])
                    };
                  };
                };
                */
